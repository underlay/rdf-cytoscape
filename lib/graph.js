...
function createNode(term, nodes, elements) {
    const id = n3_ts_1.toId(term);
    if (!nodes.has(id)) {
...
function makeElements(activeCtx, quads) {
    const compact = (iri, vocab) => {
        try {
            return compact_1.compactIri({ activeCtx, iri, relativeTo: { vocab: !!vocab } });
        }
        catch (e) {
            return iri;
        }
    ...
    for (const [index, quad] of quads.entries()) {
        ...
        const subjectId = n3_ts_1.toId(subject);
        const objectId = n3_ts_1.toId(object);
        ...
        if (object.termType === "Literal") {
        ...
        else if (object.termType === "NamedNode" && iri === n3_ts_1.IRIs.rdf.type) {
            ...
            elements.push({
                ...
                data: {
                    ...
                    source: utils_1.encode(subjectId),
                    target: utils_1.encode(objectId),
                },
            });
        }
    }

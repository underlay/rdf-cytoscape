...
function Dataset({ store, context, focus, onFocus }) {
    ...
    const defaultGraphSize = React.useMemo(() => store.countQuads(null, null, null, n3_1.DataFactory.defaultGraph()), [store]);
    ...
    const handleMouseOver = React.useCallback((id) => {
        for (const [graph, cy] of cys.current.entries()) {
            if (id !== "") {
                cy.$("#" + utils_1.encode(id)).addClass("hover");
            }
            if (id === graph) {
    ...
    const handleMouseOut = React.useCallback((id, graph) => {
        if (id === null) {
            ...
            for (const [graph, cy] of cys.current.entries()) {
                if (id !== "") {
                    cy.$("#" + utils_1.encode(id)).removeClass("hover");
                }
                if (id === graph) {
    ...
    const renderGraph = (graph) => (React.createElement(graph_1.default, { key: graph, focus: focus, graph: graph, store: store, context: context, onSelect: handleSelect, onUnselect: handleUnselect, onMouseOver: handleMouseOver, onMouseOut: (id) => handleMouseOut(id, graph), onMount: (cy) => cys.current.set(graph, cy), onDestroy: () => cys.current.delete(graph) }));
    if (graphs.length === 0) {
        return renderGraph("");
    }
    if (defaultGraphSize === 0) {
        return (React.createElement(react_panelgroup_1.default, { direction: "column", borderColor: utils_1.BorderColor, spacing: 1, onUpdate: handleInnerUpdate }, graphs.map(renderGraph)));
    }

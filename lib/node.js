...
export default function Node(id, types, literals, compact) {
    ...
    const width = 2 * 6 +
        CHAR *
            Math.max(TAB, name === null ? 0 : getLength(name), properties + TAB + Math.max(rdfTypes, dataTypes + TAB + dataValues));
    const lines = ["", STYLE];
    let height = 2;
    if (name !== null) {
    ...
    if (types.length > 0 || literalKeys.length > 0) {
        ...
        if (types.length) {
            ...
            for (let i = 0; i < compactRDFTypes.length; i++) {
                lines.push(renderTerm(compactRDFTypes[i], propertiesOffset, top + LINE_HEIGHT * i));
            }
            if (literals.size) {
        ...
        const h = compactKeys.reduce((j, term, i) => {
            ...
            for (let k = 0; k < compactDataTypes[i].length; k++) {
                lines.push(renderTerm(compactDataTypes[i][k], propertiesOffset, top + (k + j) * LINE_HEIGHT, "t"));
            }
            const l = literalValues[i];
            for (let k = 0; k < l.length; k++) {
                lines.push(renderLiteral(l[k], propertiesOffset + (dataTypes + TAB) * CHAR, top + (k + j) * LINE_HEIGHT));
            }

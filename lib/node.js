...
function Node(id, types, literals, compact) {
    ...
    const width = 2 * 6 +
        utils_1.CHAR *
            Math.max(utils_1.TAB, getLength(name), properties + utils_1.TAB + Math.max(rdfTypes, dataTypes + utils_1.TAB + dataValues));
    const lines = ["", STYLE, renderTerm(name, 6, 16)];
    let height = utils_1.LINE_HEIGHT + 2;
    if (types.length || literals.size) {
        lines.push(`<line x1="6" y1="22" x2="${width -
        ...
        if (types.length) {
            ...
            if (literals.size) {
                ...
                top = y + 16;
                lines.push(`<line x1="${propertiesOffset}" y1="${y}" x2="${width -
